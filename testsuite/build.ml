...
|}
  ...
  let compile (tgt : t) ~(input : Path.t) ~(prefix : Path.t) : [ `Compile ] artifact =
...
|}
  ...
  let assemble tgt ~(artdir : Path.t) ~(art : [ `Compile ] artifact)
    : [ `Assemble ] artifact
    =
    let name = String.chop_suffix_if_exists art.name ~suffix:".out" ^ ".o" in
    let input = Path.append_part artdir (PPart.of_string art.name) in
...
|}
  ...
  (** Link object file with stdlib to produce executable *)
  let link tgt ~(stdlib : Path.t) ~(artdir : Path.t) ~(art : [ `Assemble ] artifact)
    : [ `Link ] artifact
    =
    let name = String.chop_suffix_if_exists art.name ~suffix:".o" ^ ".exe" in
    let input = Path.append_part artdir (PPart.of_string art.name) in
    let cmd = Path.to_string tgt.link_cmd in
    let rule =
      spf link_rule name (Path.to_string stdlib) (Path.to_string input) cmd cmd
    in
    { name; rule }
  ;;
...
|}
  ...
  let run
    ...
    : [ `Run ] artifact
    =
    let name = String.chop_suffix_if_exists art.name ~suffix:".exe" ^ ".t" in
    let input = Path.append_part artdir (PPart.of_string art.name) in
...
module Test = struct
  ...
  let gen test ~root : dune_rules =
    ...
    let f acc tgt_name =
      ...
      let assembled =
        Target.assemble tgt ~artdir:(Path.of_string "../expected") ~art:compiled
      in

      let stdlib =
        Path.of_string (spf "back/%s/rukaml_stdlib.o" (Target.show_name tgt.name))
      in
      let stdlib = Path.append Path.dot_dot @@ Path.append root stdlib in

      let linked = Target.link tgt ~stdlib ~artdir:Path.dot ~art:assembled in
      ...
      let artifacts, cram =
        ...
        | Some { stdout; exit } ->
          let art = Target.run tgt ~stdout ~exit ~artdir:Path.dot_dot ~art:linked in
          let cram_rule =
            spf
